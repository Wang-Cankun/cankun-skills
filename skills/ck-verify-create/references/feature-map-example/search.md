...
- **Empty state.** Reopen search with `control-notes browser click --role button --name "Search"`, then enter `volcano` with `control-notes browser fill --role searchbox --name "Search notes" --value "volcano"`. A status named `No matching notes` appears after search completes.
...
- **Proof.** Restore the populated result state with `control-notes browser fill --role searchbox --name "Search notes" --value "quarterly"` and wait for `Quarterly plan` in `Search results`. Run `control-notes browser snapshot --aria --path artifacts/search/results.aria.txt` and `control-notes browser screenshot --path artifacts/search/results.png`. Both artifacts identify Notes, the query, and `Quarterly plan`.
