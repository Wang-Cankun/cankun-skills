...
- **Keyboard entry.** From the note list, focus outside editable fields and run `control-notes browser press --key "n"`. A blank `Note editor` appears with focus in the `Title` textbox. Run `control-notes browser click --role button --name "Cancel"` to return to the list without saving.
...
- **Proof.** From the list, open `CLI note` and assert its saved title and body, retaining an editor snapshot. Return via `All notes` after checking each saved note. With the list visible, run `control-notes browser snapshot --aria --path artifacts/create-note/list.aria.txt` and `control-notes browser screenshot --path artifacts/create-note/list.png`. These list artifacts show both `Release checklist` and `CLI note`; the earlier editor checks establish their saved contents.
