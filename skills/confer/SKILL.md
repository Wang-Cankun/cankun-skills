...
You talk to a peer model through **threads**: open one with a question, the peer's session id is stored, and any later round — today or next week, from any host session — resumes the same peer-side context. All mechanics live in `scripts/confer.sh` (single source of truth); never assemble raw `claude -p` / `codex exec` calls yourself.

```
scripts/confer.sh open <provider> [-t name] <prompt|->   # start thread (claude|codex)
scripts/confer.sh reply <thread> <prompt|->              # continue with full peer-side context
scripts/confer.sh all <prompt|->                         # fan one question out to every provider
scripts/confer.sh list | show <thread>                   # registry / transcript
scripts/confer.sh doctor [--live]                        # health check (run --live once after install)
```
